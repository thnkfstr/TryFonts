...
{
  public partial class TryFonts : Form
  {
    private int loadedFontCount = 0;
    private const int LoadIncrement = 15; // Number of fonts to load at once (page size)
    ...
    public TryFonts()
    {
      ...
      //// Set focus to nothing initially
      //this.Load += (s, e) =>
      //{
      //  this.ActiveControl = null; // Ensure no control is focused
      //  tryTextBox.SelectionStart = tryTextBox.Text.Length; // Ensure text is not highlighted
      //};

      // Set focus to a hidden button to prevent other elements from receiving focus
      ...
      try
      {
        var filteredFonts = cachedFonts
          .Where(f => string.IsNullOrEmpty(searchText) ||
              (containsOrStartsWithComboBox.SelectedItem.ToString() == "Starts with" && f.Name.ToLower().StartsWith(searchText)) ||
              (containsOrStartsWithComboBox.SelectedItem.ToString() == "Contains" && f.Name.ToLower().Contains(searchText)))
        ...
        if (IsHandleCreated && !IsDisposed)
        {
          Invoke(new Action(() =>
          {
            ...
            if (scrollablePanel.VerticalScroll.Maximum > 0)
            {
        ...
        // Try again to make sure all the right fonts display when initially loaded
        Invoke(new Action(() =>
        {
          fontViewingContainer.PerformLayout(); // Force layout update after loading fonts
          fontViewingContainer.Refresh(); // Force a redraw of the container to fix alignment issues
          fontViewingContainer.Invalidate(); // Forcefully invalidate to trigger repaint
          fontViewingContainer.Update(); // Update ensures the invalidated areas are repainted immediately
        }));

        // Print the fonts to the console so I can see the order they were added
        foreach (var fontFamily in filteredFonts)
        {
          Console.WriteLine($"Loading font: {fontFamily.Name}"); // Debug line to check the order of loaded fonts
                                                                 // Your existing code for adding fonts here
        }
      }
    ...
    {
      // Ensure input is treated as text
      ...
      {
        Text = sanitizedText, // Directly treat input as plain text
        Font = font,
        AutoSize = true,
